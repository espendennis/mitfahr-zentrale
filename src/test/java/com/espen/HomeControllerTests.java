...
public class HomeControllerTests {
	...
	public void testOfferCreated() throws Exception{
		mockMvc.perform(get("/offercreated").with(user("admin").password("123456seven")))
		.andExpect(status().isOk())
	...
	public void testUserCreated() throws Exception{
		mockMvc.perform(get("/usercreated").with(user(user2)))
		.andExpect(status().isOk())
	...
	public void testShowOffer() throws Exception{
		mockMvc.perform(get("/offer/{id}", offer1.getId()))
		.andExpect(status().isOk())  
	...
	public void testLoggingIn() throws Exception{
		mockMvc.perform(formLogin().user("admin").password("123456seven"))
	...
	public void testLogginFail() throws Exception{
		mockMvc.perform(formLogin().user("notExistent").password("asdasdad"))
	...
	public void testNewOffer() throws Exception{
		...
		mockMvc.perform(post("/newoffer")
				.with(user("admin").password("123456seven"))
	...
	public void testOwnProfile() throws Exception{
		mockMvc.perform(get("/ownprofile").with(user(user2)))
		.andExpect(status().isFound())
	...
	public void testNewUser() throws Exception{
		User testuser = new User("newuser", "Washington", "George", "Constitution", "george@thewhitehouse.gov", "01435673423");
		mockMvc.perform(post("/newuser")
				...
				.contentType(MediaType.APPLICATION_FORM_URLENCODED))
		.andExpect(status().isFound())
		.andExpect(view().name("redirect:/usercreated"));
	...
	public void testCreateOffer() throws Exception{
		mockMvc.perform(get("/createoffer").with(user(user2)))
		.andExpect(status().isOk())
	...
	public void testProfile() throws Exception{
		mockMvc.perform(get("/profile/{username}", user2.getUsername()).with(user(user2)))
			.andExpect(status().isOk())
